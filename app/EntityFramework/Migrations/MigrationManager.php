...
PHP;
    ...
    {
        try {
            $generator = new \App\EntityFramework\Migrations\MigrationGenerator();
            $result = $generator->generateMigrationCode();
            
            // Debug: Log the result
            error_log('generateMigrationFromContext: result type: ' . gettype($result));
            if (is_array($result)) {
                error_log('generateMigrationFromContext: result keys: ' . implode(', ', array_keys($result)));
                error_log('generateMigrationFromContext: up length: ' . strlen($result['up'] ?? ''));
                error_log('generateMigrationFromContext: down length: ' . strlen($result['down'] ?? ''));
                error_log('generateMigrationFromContext: up trimmed length: ' . strlen(trim($result['up'] ?? '')));
                error_log('generateMigrationFromContext: down trimmed length: ' . strlen(trim($result['down'] ?? '')));
            }
            
            // Check if result is valid and return it
            if (is_array($result) && isset($result['up']) && isset($result['down'])) {
                $upTrimmed = trim($result['up']);
                $downTrimmed = trim($result['down']);
                
                error_log('generateMigrationFromContext: checking lengths - up: ' . strlen($upTrimmed) . ', down: ' . strlen($downTrimmed));
                
                if (strlen($upTrimmed) > 0 && strlen($downTrimmed) > 0) {
                    error_log('generateMigrationFromContext: returning result');
                    return $result;
                } else {
                    error_log('generateMigrationFromContext: trimmed code is empty, returning null');
                }
            } else {
                error_log('generateMigrationFromContext: result is not valid array or missing keys');
            }
            
            return null;
        } catch (\Exception $e) {
            error_log('Failed to generate migration from context: ' . $e->getMessage());
            error_log('Stack trace: ' . $e->getTraceAsString());
            return null;
        } catch (\Throwable $e) {
            error_log('Failed to generate migration from context (Throwable): ' . $e->getMessage());
            error_log('Stack trace: ' . $e->getTraceAsString());
            return null;
        }
    }
